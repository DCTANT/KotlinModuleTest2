package com.example.dctlib.widget.container;
...
public class DefaultHeader extends BaseHeader {
    ...
    public void onPreDrag(View rootView) {
        if (freshTime==0){
            ...
            if(m>=1 && m<60){
                headerTime.setText( m +rootView.getResources().getString(R.string.minutes_ago));
            }else if (m>=60){
                int h = m/60;
                headerTime.setText( h +rootView.getResources().getString(R.string.hours_ago));
            }else if(m>60*24){
                int d = m/(60*24);
                headerTime.setText( d +rootView.getResources().getString(R.string.days_age));
            }else if(m==0){
                headerTime.setText(rootView.getResources().getString(R.string.just_now));
            }
        }
    }
    ...
    public void onLimitDes(View rootView, boolean upORdown) {
        if (!upORdown){
            headerTitle.setText(context.getResources().getString(R.string.release_to_refresh));
            if (headerArrow.getVisibility()== View.VISIBLE)
                headerArrow.startAnimation(mRotateUpAnim);
        }
        else {
            headerTitle.setText(context.getResources().getString(R.string.pull_down_to_refresh));
            if (headerArrow.getVisibility()== View.VISIBLE)
    ...
    public void onStartAnim() {
        freshTime = System.currentTimeMillis();
        headerTitle.setText(context.getString(R.string.refreshing));
